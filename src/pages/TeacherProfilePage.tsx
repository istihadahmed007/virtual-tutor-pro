    ...
    return (
      <main className="min-h-screen bg-[#FAFAF8] flex items-center justify-center">
        <EmptyState
          ...
          description="This teacher profile doesn't exist or hasn't been set up yet."
            ...
            <div className="flex-1">
              ...
              <div className="flex items-center gap-2 mt-3 flex-wrap">
                {teacher.languages.map((l) => (
                  <span
                    key={l}
                    className="inline-flex items-center gap-1 px-2 py-0.5 bg-stone-100 text-slate-600 text-xs font-medium rounded-full"
                  >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            <Card className="border-stone-200/80">
              ...
              <CardContent>
                <div className="relative overflow-hidden rounded-xl bg-gradient-to-br from-stone-100 to-stone-50 aspect-video flex items-center justify-center">
                  <div className="text-center">
                    <div className="w-16 h-16 rounded-full bg-teal-50 flex items-center justify-center mx-auto mb-3 border-2 border-teal-100">
                      <Play className="w-6 h-6 text-teal-600 ml-0.5" />
                    </div>
                    <p className="text-sm font-semibold text-slate-700">
                      Watch {teacher.name.split(" ")[0]}'s intro video
                    </p>
                    <p className="text-xs text-slate-400 mt-1">
                      Learn about their teaching style and approach
                    </p>
                  </div>
                </div>
              </CardContent>
              ...
              <CardContent>
                {reviewList.length === 0 ? (
                  <EmptyState
                    ...
                    description="This teacher hasn't received any reviews yet. Be the first to book a session!"
                  />
                ) : (
            ...
            {bookingConfirmed ? (
              <div className="p-8 text-center">
                ...
                <p className="text-sm text-slate-500 mt-2">
                  Your session with {teacher.name} has been confirmed.
                </p>
                ...
                <div className="flex items-center gap-2 mb-6">
                  {[1, 2, 3].map((s) => (
                    <div key={s} className="flex items-center gap-2 flex-1">
                      <div
                        className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${bookingStep >= s ? "bg-teal-600 text-white" : "bg-stone-100 text-slate-400"}`}
                      >
                        {s}
                      </div>
                      {s < 3 && (
                        <div
                          className={`flex-1 h-0.5 ${bookingStep > s ? "bg-teal-600" : "bg-stone-200"}`}
                        />
                      )}
                    </div>
                  ))}
                ...
                {bookingStep === 1 && (
                  <div>
                    ...
                    <p className="text-sm text-slate-500 mb-4">
                      Select the type of session you want
                    </p>
                    <div className="space-y-2">
                      {sessionTypes.map((type) => (
                        <button
                          ...
                          className="w-full p-4 rounded-xl border-2 text-left transition-all border-stone-200 hover:border-teal-200"
                        >
                      ...
                      {dates.map((d) => {
                        ...
                        return (
                          <button
                            ...
                            className={`p-2 rounded-lg border text-xs font-medium text-center transition-all ${selectedDate === d ? "border-teal-500 bg-teal-50 text-teal-700" : "border-stone-200 text-slate-600 hover:border-teal-200"}`}
                          >
                    ...
                    <div className="grid grid-cols-3 gap-2 mb-6">
                      {times.map((t) => (
                        <button
                          ...
                          className={`p-2 rounded-lg border text-xs font-medium text-center transition-all ${selectedTime === t ? "border-teal-500 bg-teal-50 text-teal-700" : "border-stone-200 text-slate-600 hover:border-teal-200"}`}
                        >
                          {t}
                        </button>
                      ))}
