  ...
  return (
    <main className="min-h-screen bg-[#FAFAF8]">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 lg:py-8">
        ...
        {profileStatus && !profileStatus.isComplete && profileStatus.role === "teacher" && (
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 mb-6 flex items-center justify-between">
            <div>
              <p className="text-sm font-bold text-amber-800">Complete your teacher profile</p>
              <p className="text-xs text-amber-600 mt-1">Your profile is {profileStatus.completionPercentage}% complete. Complete it to get verified and start teaching.</p>
            </div>
            <Button size="sm" className="bg-amber-600 hover:bg-amber-700 text-white" onClick={() => navigate("/teacher-dashboard")}>Complete Profile</Button>
          </div>
        )}
        ...
        {nextSession && (
          <div className="bg-gradient-to-br from-teal-600 to-teal-700 rounded-2xl p-6 text-white mb-6 shadow-xl shadow-teal-600/15">
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
              <div>
                <p className="text-xs font-bold text-teal-100 uppercase tracking-wider mb-1">Next Class</p>
                <h2 className="text-2xl font-extrabold">{nextSession.title}</h2>
                <p className="text-teal-200 text-sm mt-1">
                  with {nextSession.teacherName} • {nextSession.durationMinutes} min • {nextSession.sessionType}
                </p>
              </div>
              <Button
                onClick={() => navigate(`/classroom?session=${nextSession._id}`)}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-2xl border border-stone-200/80 p-5">
              <h3 className="text-base font-bold text-slate-900 flex items-center gap-2 mb-4">
                <Calendar className="w-4 h-4 text-teal-500" /> Upcoming Classes
              </h3>
              {upcomingSessions.length === 0 ? (
                <EmptyState icon={Video} title="No upcoming classes" description="Find a teacher and book your first live session to get started." actionLabel="Find a Teacher" actionPath="/teachers" />
              ) : (
                <div className="space-y-3">
                  {upcomingSessions.slice(0, 5).map((session) => {
                    const date = new Date(session.scheduledAt);
                    return (
                      <div key={session._id} className="flex items-center gap-4 p-4 bg-stone-50 rounded-xl hover:bg-teal-50/50 transition-colors cursor-pointer" onClick={() => navigate(`/classroom?session=${session._id}`)}>
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl p-5 border border-stone-200/80">
              ...
              <EmptyState icon={TrendingUp} title="No progress yet" description="Your learning progress will appear after your first class." />
            </div>
          </div>
        </div>
      </div>
